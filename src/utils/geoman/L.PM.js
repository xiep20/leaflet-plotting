...
L.PM = L.PM || {
  ...
  addInitHooks() {
    ...
    function initPolygon() {
      this.pm = undefined;
      //箭头面单独判定
      if (this.options._type2 === "Arrow" || this.type === "squadcombat") {
        if (L.PM.optIn) {
          if (this.options.pmIgnore === false) {
            this.pm = new L.PM.Edit.Arrow(this);
          }
        } else if (!this.options.pmIgnore) {
          this.pm = new L.PM.Edit.Arrow(this);
        }
  ...
  reInitLayer(layer) {
    ...
    if (layer.pm) {
      ...
      // } else if (layer instanceof L.Arrow) {
      //   layer.pm = new L.PM.Edit.Arrow(layer);
