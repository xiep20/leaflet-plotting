 ...
 * 箭头绘制类
 */
Draw.Arrow = Draw.Polygon.extend({
  initialize(map) {
    this._map = map;
    this._shape = 'Arrow';
  ...
  _createMarker(latlng) {
    ...
    if (this._layer.getLatLngs().flat().length === 1) {
      ...
      // }
    } else if (this._layer.getLatLngs().flat().length === 3) {
      this._finishShape()
      if (this.options.snappable) {
        this._cleanupSnapping();
      }
  ...
  _finishShape() {
    ...
    for (let c = 0; c < coords.length; c++) {
      poltCoords.push([coords[c].lat, coords[c].lng])
    }
    //根据点绘制箭头面
    const polygonLayer = L.Plot.squadCombat(poltCoords, this.options.pathOptions)
