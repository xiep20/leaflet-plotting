...
export default class WebRtcPlayer {
    ...
    async handleNegotiationNeeded() {
        let self = this
        let offer = await self.webrtc.createOffer();
        await self.webrtc.setLocalDescription(offer);
        ...
        let _data = { data: btoa(self.webrtc.localDescription.sdp) };
        axios.post(self.rsdpLink, qs.stringify(
            _data
        ))
            .then(function (data) {
                try {
                    self.webrtc.setRemoteDescription(new RTCSessionDescription({
                        type: 'answer',
                        sdp: atob(data.data)
                    }))
                } catch (e) {
